 ...
 *
 * Current Integration:
 * - Vehicle handling (`handleVehicle`) is integrated, managing normal and emergency
 *   vehicles.
 * - Queue management is functional; queues are cleared when they reach maximum length.
 * - Emergency response (`setAllSignalsRed`) is triggered for emergency vehicles and errors.
 *
 * Unused or Underutilized Features:
 * - Yellow Signal Transitions: The system currently toggles directly between red and green
 *   signals. Adding yellow transitions would make signal changes more realistic.
 * - Error Codes: While errors can trigger system-wide red signals, specific error codes
 *   generated by `ErrorHandler` are not actively logged or displayed for diagnostics.
 */
